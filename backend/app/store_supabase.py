...
class SupabaseStore:
    ...
    def _to_brand_profile(self, data: dict[str, Any]) -> BrandProfile:
        ...
        if payload.get("brand_facts") is None:
            payload["brand_facts"] = {}
    ...
    async def host_dashboard_stats(self, user_id: str) -> HostDashboardStats:
        ...
        open_polls, open_prompts, questions, polls_all = await asyncio.gather(
            self._select(
                "polls",
    ...
    async def reset_poll_votes(
        ...
        """Zero every option's count and drop voter history, keeping the poll.

        Deleting poll_votes rows lets previous voters vote again after the
        ...
        self._invalidate_session_snapshot(session_id)
        return self._to_poll(rows[0], options)
