...
def _artifact_system_instruction_lines(kind: str) -> list[str]:
    ...
    return [
        ...
        *v["update_requirement_lines"],
        ...
        f"- For layout in the sandboxed iframe, avoid vh/vw for primary bar heights and other critical vertical sizing: the host measures document size and resizes the iframe, which changes vh/vw and can cause jitter or feedback loops. Prefer % of a fixed scene root, flex, CSS grid with minmax(0, 1fr), or clamp(..., px, ...) — not vh — for {v['vh_tail']}.",
