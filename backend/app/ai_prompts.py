...
Artifacts exist for three activity kinds (see docs/artifact-activity-kinds.md):
``poll`` (options + votes), ``qna`` (session-level audience Q&A), and
``discussion`` (a QnaPrompt the host posed; audience submits answers). The
``qna`` and ``discussion`` kinds share one runtime contract — a moderated,
upvoted list of audience text submissions — and differ only in copy. The
``build_artifact_*`` functions below assemble the per-kind instructions from a
shared skeleton; the legacy ``POLL_GAME_*`` constants remain the poll
instances so existing imports and tests keep working.
"""
...
ARTIFACT_ACTIVITY_KINDS = ("poll", "qna", "discussion")

# EmbedInstance.artifact_kind spellings that map onto activity kinds.
...
def normalize_artifact_activity_kind(value: Any) -> str:
    """Coerce arbitrary client input to a canonical activity kind.

    Unknown or missing values fall back to ``poll`` so every legacy caller
    (which never sent a kind) keeps its exact previous behavior.
    """
    if isinstance(value, str):
        kind = value.strip().lower()
        if kind in ARTIFACT_ACTIVITY_KINDS:
            return kind
        if kind in _ARTIFACT_KIND_ALIASES:
            return _ARTIFACT_KIND_ALIASES[kind]
    return "poll"


def artifact_activity_family(activity_kind: Any) -> str:
    """Runtime-contract family: ``qna`` and ``discussion`` share the
    question-list contract, so kind-family checks should dispatch through
    this instead of re-encoding the membership set at each site."""
    return "qna" if normalize_artifact_activity_kind(activity_kind) in ("qna", "discussion") else "poll"
...
# Per-kind fragments for the artifact build/edit/repair system instruction.
# The "poll" entries reproduce the original poll-game instruction verbatim so
# poll behavior is unchanged; "qna" and "discussion" are the audience-text
# parallels. Only genuinely kind-dependent lines live here — everything else
# is shared skeleton in _artifact_system_instruction_lines.
_ARTIFACT_KIND_VOCAB: dict[str, dict[str, Any]] = {
    "poll": {
        "head": "You build complete interactive HTML artifacts for a live poll game canvas.",
        "contract_lines": [
        ...
        "structure_noun": "option row structure",
        "flicker_fix_line": "- If the user asks to reduce flicker, stop resets, or improve animation continuity, preserve the current DOM tree and animate existing option elements forward with transform/transition updates keyed by option id.",
        "still_consume_line": "- The edited artifact must still consume host-delivered live poll state and must still call window.prezoSetPollRenderer(fn), define window.prezoRenderPoll(state), or use an equivalent runtime-approved render registration hook from the existing host contract.",
        "first_render_line": "- The returned artifact must remain immediately usable after first render: visible poll scene, readable labels, and no empty, hidden, or near-solid full-screen overlay obscuring the content unless the user explicitly asks for that.",
        "update_requirement_lines": [
        ...
        "chrome_line": "- Assume base poll chrome can be replaced by your artifact scene composition.",
        "vh_tail": "vote bars and main columns",
        "readability_line": "- Prioritize readability at all times: titles, poll labels, values, and motion should remain easy to understand at a glance.",
        "animation_line": "- Use animation with purpose: smooth, cinematic, and responsive to vote changes, but not noisy or distracting.",
        "clarity_line": "- When interpreting stylized prompts, preserve functional poll communication instead of sacrificing clarity for aesthetics.",
        "scaling_lines": [
    ...
    "qna": {
        ...
        "contract_lines": [
            '- Host posts message: { "type":"prezo-qna-state", "payload": state }',
            "- State shape: state.kind ('qna' or 'discussion'), state.qna.title, state.qna.status, state.qna.questions[], state.qna.totalQuestions, state.qna.totalVotes, state.meta.",
            "- Each question: { id, text, votes, percentage, rank } — approved audience submissions only, already sorted by upvotes then recency; percentage is that question's share of all upvotes.",
            "- state.qna.title is the session Q&A heading. Feature it as the board's title.",
            "- If available, use state.meta.recommendedVisibleQuestions and state.meta.expectedMaxQuestions to cap the visible list and design an overflow indicator (e.g. '+N more questions').",
            "- The question list usually starts empty. Design a compelling empty state that invites the audience to participate — never a blank or broken-looking stage.",
            "- Prefer registering your renderer with window.prezoSetQnaRenderer(fn) when available. You may also define window.prezoRenderQna(state).",
            "- Do not implement your own window message listener or websocket logic for Q&A updates unless the user explicitly asks.",
        ],
        "structure_noun": "question list structure",
        "flicker_fix_line": "- If the user asks to reduce flicker, stop resets, or improve animation continuity, preserve the current DOM tree and animate existing question elements forward with transform/transition updates keyed by question id.",
        "still_consume_line": "- The edited artifact must still consume host-delivered live Q&A state and must still call window.prezoSetQnaRenderer(fn), define window.prezoRenderQna(state), or use an equivalent runtime-approved render registration hook from the existing host contract.",
        "first_render_line": "- The returned artifact must remain immediately usable after first render: visible Q&A scene, readable question text, and no empty, hidden, or near-solid full-screen overlay obscuring the content unless the user explicitly asks for that.",
        "update_requirement_lines": [
            "- Question list changes must animate smoothly (about 200ms-500ms easing) with no flicker.",
            "- Do not rebuild or re-mount the full scene on each update.",
            "- Never blank the stage between updates and never use hide-then-show, fade-to-black, blackout overlays, or other hard reset transitions unless the user explicitly asks for that effect.",
            "- Build around a stable scene root and persistent question nodes keyed by question id.",
            "- Reconcile by question id and update only changed elements when possible.",
            "- Renderer idempotence is required: repeated calls with the same or newer state must not increase question-row count or duplicate entries.",
            "- New questions should enter with a clear but non-disruptive animation, hidden questions should exit gracefully, and rank changes should animate vertical movement with transforms on stable mounted nodes.",
            "- Do not reinsert or reorder every existing lane/row node with appendChild/removeChild on each update.",
            "- If the scene contains moving objects such as cards, bubbles, avatars, or tokens, keep the same DOM nodes mounted and animate them forward from prior state instead of destroying and recreating them.",
        ],
        "chrome_line": "- Assume base Q&A chrome can be replaced by your artifact scene composition.",
        "vh_tail": "question rows and main columns",
        "readability_line": "- Prioritize readability at all times: the title, question text, vote counts, and motion should remain easy to understand at a glance.",
        "animation_line": "- Use animation with purpose: smooth, cinematic, and responsive to new submissions and vote changes, but not noisy or distracting.",
        "clarity_line": "- When interpreting stylized prompts, preserve functional Q&A communication instead of sacrificing clarity for aesthetics.",
        "scaling_lines": [
            "- Design the layout to scale from 0 to many questions: cap the visible list, rank the rest behind an overflow count, and keep the composition balanced at every list length.",
            "- Audience questions are free text of unpredictable length. Wrap or clamp long text gracefully (multi-line with ellipsis) so a single long question can never break the layout.",
            "- Always preserve exact upvote counts in text even when the visual uses size, bars, or emphasis to express popularity.",
            "- Render audience question text verbatim (aside from length clamping). Never invent, paraphrase, reword, or reorder submissions.",
        ],
...
# Discussion shares the qna contract; only the framing copy differs.
_ARTIFACT_KIND_VOCAB["discussion"] = {
    **_ARTIFACT_KIND_VOCAB["qna"],
    "head": "You build complete interactive HTML artifacts for a live open-discussion board.",
    "contract_lines": [
        line
        if not line.startswith("- state.qna.title is")
        else "- state.qna.title is the discussion prompt the host posed. Keep it prominent — it frames every audience submission."
        for line in _ARTIFACT_KIND_VOCAB["qna"]["contract_lines"]
    ],
    "first_render_line": "- The returned artifact must remain immediately usable after first render: visible discussion scene, readable prompt and submissions, and no empty, hidden, or near-solid full-screen overlay obscuring the content unless the user explicitly asks for that.",
}


def _artifact_system_instruction_lines(kind: str) -> list[str]:
    v = _ARTIFACT_KIND_VOCAB[kind]
    return [
        v["head"],
        ...
        *v["contract_lines"],
        ...
        f"- Do not rewrite the full document, <body>, primary scene root, or {v['structure_noun']} unless the user explicitly asks for a structural redesign.",
        ...
        *v["update_requirement_lines"],
        ...
        v["archetype_line"],
        "- Illustrated objects (vehicles, avatars, buildings, mascots, trophies) must be drawn as inline SVG with clean, well-proportioned geometry — never as emoji characters, unicode glyphs, or box-shadow/div pixel art. Every object must read at a glance; if you cannot draw it convincingly in a few SVG shapes, use a clean abstract marker (a colored token, a badge, a simple silhouette) instead.",
        "- In progress or race archetypes, design the environment itself, not just the movers: lanes get a visible surface with markings and a designed start and finish, and backdrop elements (skylines, crowds, scenery) are deliberate silhouette compositions with varied shapes, never rows of plain gray rectangles.",
        ...
        "- Reserve layout room for extreme data states: an item at 0% and at 100% must both render cleanly, with no labels, values, markers, or avatars overlapping each other or the end-of-lane ornament. Test your layout mentally at 0%, 50%, and 100%.",
        "- Light and glow layers (suns, halos, spotlights, bloom) sit behind content and must never overlap or wash out text and data.",
        ...
        "- The same rescale breaks JavaScript-cached pixel measurements: position progress movers, markers, and data-driven offsets as percentages of their lane/container (left: N%, or transform: translateX(...) computed from a percentage), never from a container width measured once at load. If a pixel measurement is truly unavoidable, re-measure it inside the renderer on every update and attach a ResizeObserver to the container so positions recompute when the host resizes or enters present mode.",
        *v["scaling_lines"],
...
def _artifact_system_instruction_for(kind: str) -> str:
    return "\n".join(_artifact_system_instruction_lines(kind))


def build_artifact_system_instruction(activity_kind: str = "poll") -> str:
    """Build/edit/repair system instruction for the given activity kind.

    Cached per canonical kind: the poll instruction used to be an import-time
    constant, and build requests re-request it up to five times each."""
    return _artifact_system_instruction_for(normalize_artifact_activity_kind(activity_kind))


POLL_GAME_ARTIFACT_SYSTEM_INSTRUCTION = build_artifact_system_instruction("poll")


_ARTIFACT_ASSISTANT_LIVE_DATA_NOUN = {
    "poll": "its live poll data",
    "qna": "its live audience Q&A data",
    "discussion": "its live discussion data",
}


def build_artifact_assistant_system_instruction(activity_kind: str = "poll") -> str:
    return _artifact_assistant_system_instruction_for(
        normalize_artifact_activity_kind(activity_kind)
    )


@lru_cache(maxsize=None)
def _artifact_assistant_system_instruction_for(kind: str) -> str:
    return "\n".join(
        [
            "You are a text assistant for the Prezo artifact editor.",
            f"Answer questions about the current artifact, its behavior, {_ARTIFACT_ASSISTANT_LIVE_DATA_NOUN[kind]}, and likely causes of issues.",
...
POLL_GAME_ARTIFACT_ASSISTANT_SYSTEM_INSTRUCTION = (
    build_artifact_assistant_system_instruction("poll")
)


_ARTIFACT_PATCH_DYNAMIC_ELEMENTS_LINE = {
    "poll": (
    ...
    "qna": (
        '- { "type":"insert_html", "target": string, "position": "beforeend"|"afterbegin"|"beforebegin"|"afterend", "html": string }  — inserts HTML snippet relative to the first element matching "target" (a simple CSS selector: tag, #id, .class, or [attr]). No <script> tags or on* attributes allowed. Use this to add new visual elements (clouds, stars, decorations, SVG shapes, etc.). IMPORTANT: insert_html only operates on the static index.html file. Question list elements (cards, rows, question text, vote badges) are created dynamically by renderer.js at runtime and do NOT exist in index.html. Do not use insert_html with selectors that target JS-generated question elements (e.g. .question-row, .question-card, .vote-badge) — those selectors will not be found. To add per-question markup, you must output a full artifact rewrite that modifies the renderer JS where question nodes are built.'
    ),
}
_ARTIFACT_PATCH_DYNAMIC_ELEMENTS_LINE["discussion"] = _ARTIFACT_PATCH_DYNAMIC_ELEMENTS_LINE["qna"]

_ARTIFACT_PATCH_LIVE_WIRING_NOUN = {
    "poll": "live poll wiring",
    "qna": "live Q&A wiring",
    "discussion": "live Q&A wiring",
}


def build_artifact_patch_system_instruction(activity_kind: str = "poll") -> str:
    return _artifact_patch_system_instruction_for(
        normalize_artifact_activity_kind(activity_kind)
    )


@lru_cache(maxsize=None)
def _artifact_patch_system_instruction_for(kind: str) -> str:
    return "\n".join(
        [
            f"You generate minimal JSON patch plans for an existing {'live poll artifact' if kind == 'poll' else 'live audience Q&A artifact'}.",
            ...
            _ARTIFACT_PATCH_DYNAMIC_ELEMENTS_LINE[kind],
            ...
            f"- Preserve unrelated HTML, CSS, JavaScript, SVG, ids, classes, data attributes, and {_ARTIFACT_PATCH_LIVE_WIRING_NOUN[kind]} exactly.",
...
POLL_GAME_ARTIFACT_PATCH_SYSTEM_INSTRUCTION = build_artifact_patch_system_instruction(
    "poll"
)
