...
export function buildTextStyleBridgeLines() {
  return [
    ...
    // The primary selection stays in selectedNode/selectedKind (handles,
    // label, host messages, resize). Additional Ctrl-clicked objects live in
    // extraSelectedNodes as {node, classification} and get plain dashed
    // outline boxes; group drag and delete include them.
    '  var extraSelectedNodes = []',
    '  var extraOverlayEls = []',
    '  function findExtraIndex(node) {',
    '    for (var i = 0; i < extraSelectedNodes.length; i++) {',
    '      if (extraSelectedNodes[i].node === node) return i',
    '    }',
    '    return -1',
    '  }',
    '  function clearExtraSelections() {',
    '    if (extraSelectedNodes.length) extraSelectedNodes = []',
    '    syncExtraOverlays()',
    '  }',
    '  function syncExtraOverlays() {',
    '    if (!document.body) return',
    // Prune entries whose nodes left the DOM (AI rebuilds, deletes).
    '    for (var d = extraSelectedNodes.length - 1; d >= 0; d--) {',
    '      var pruned = extraSelectedNodes[d]',
    '      if (!pruned.node || !document.body.contains(pruned.node)) extraSelectedNodes.splice(d, 1)',
    '    }',
    '    while (extraOverlayEls.length < extraSelectedNodes.length) {',
    '      var box = document.createElement("div")',
    '      box.setAttribute("data-prezo-selection-overlay", "extra")',
    '      box.style.cssText = [',
    '        "position:absolute",',
    '        "pointer-events:none",',
    '        "z-index:2147483646",',
    ...
    '      extraOverlayEls.push(box)',
    '    }',
    '    for (var k = 0; k < extraOverlayEls.length; k++) {',
    '      var boxEl = extraOverlayEls[k]',
    '      var entry = extraSelectedNodes[k]',
    '      if (!entry) { boxEl.style.display = "none"; continue }',
    '      var r = getDocumentRect(entry.node)',
    '      if (!r || r.w <= 0 || r.h <= 0) { boxEl.style.display = "none"; continue }',
    ...
    '    }',
    '  }',
    '  function setSelectedNode(node, classification, opts) {',
    // A plain (non-additive) selection change resets any multi-selection.
    '    if (!opts || !opts.keepExtras) clearExtraSelections()',
    ...
    // Ctrl/Cmd+click adds to (or removes from) the multi-selection.
    ...
    '        if (drilled) {',
    '          commitClickSelection(drilled.node, drilled.classification, additive)',
    '          return',
    '        }',
    '      } else {',
    '        drillScopeNode = null',
    ...
    '      if (fallback) {',
    '        commitClickSelection(target, fallback, additive)',
    '        return',
    '      }',
    // Ctrl+click on empty space keeps the current multi-selection.
    '      if (!additive) setSelectedNode(null, null)',
    '      return',
    '    }',
    '    commitClickSelection(pick.node, pick.classification, additive)',
    '  }',
    ...
    '  }',
    // Additive (Ctrl/Cmd) clicks toggle multi-selection membership instead of
    // replacing the selection. The scene root never joins a multi-selection.
    '  function commitClickSelection(node, classification, additive) {',
    '    if (!additive) {',
    '      maybePromoteSelection(node, classification)',
    '      return',
    '    }',
    '    if (!node || (classification && classification.kind === "scene")) return',
    '    if (!selectedNode) {',
    '      setSelectedNode(node, classification)',
    '      return',
    '    }',
    '    if (node === selectedNode) {',
    // Ctrl-click on the primary deselects it; the most recent extra (if
    // any) is promoted to primary so handles stay on one object.
    '      var promoted = extraSelectedNodes.length ? extraSelectedNodes[extraSelectedNodes.length - 1] : null',
    '      if (promoted) {',
    '        extraSelectedNodes.pop()',
    '        setSelectedNode(promoted.node, promoted.classification, { keepExtras: true })',
    '      } else {',
    '        setSelectedNode(null, null)',
    '      }',
    '      return',
    '    }',
    '    var idx = findExtraIndex(node)',
    '    if (idx >= 0) {',
    '      extraSelectedNodes.splice(idx, 1)',
    '      syncExtraOverlays()',
    '      return',
    '    }',
    '    extraSelectedNodes.push({ node: node, classification: classification })',
    '    syncExtraOverlays()',
    '  }',
    // ── Context menu ─────────────────────────────────────────────
    ...
    '    var roots = [selectedNode]',
    '    for (var x = 0; x < extraSelectedNodes.length; x++) {',
    '      if (extraSelectedNodes[x].node) roots.push(extraSelectedNodes[x].node)',
    '    }',
    ...
    '        if (el === roots[j] || (roots[j].contains && roots[j].contains(el))) return true',
    '      }',
    '    }',
    '    return false',
    '  }',
    ...
    '    return ""',
    '  }',
    // kindOverride/labelOverride identify multi-selection extras — the
    // selectedKind/selection label always describe the PRIMARY node.
    '  function postCommittedPosition(stableId, node, dx, dy, priorDx, priorDy, kindOverride, labelOverride) {',
    '    if (!stableId) return',
    '    var kind = kindOverride || selectedKind || ""',
    ...
    '    if (!label && selectionLabelEl && selectionLabelEl.textContent) label = selectionLabelEl.textContent',
    ...
    '    if (dragState.extras) {',
    '      for (var xi = 0; xi < dragState.extras.length; xi++) {',
    '        var ex = dragState.extras[xi]',
    '        applyPositionToNode(ex.node, ex.stableId, { dx: ex.startDx, dy: ex.startDy })',
    '        if (ex.node && ex.node.style) {',
    '          ex.node.style.transition = ex.transitionBaseline',
    '          ex.node.style.willChange = ex.willChangeBaseline',
    '        }',
    '      }',
    '    }',
    ...
    '    return true',
    '  }',
    '  function postCommittedHidden(stableId, node, kindOverride, labelOverride) {',
    '    if (!stableId) return',
    '    var meta = buildHiddenOverrideMeta(node, kindOverride || selectedKind, labelOverride)',
    ...
    '  function buildHiddenOverrideMeta(node, kind, labelOverride) {',
    '    var role = kind || ""',
    '    var optionId = (node && node.getAttribute && (node.getAttribute("data-prezo-editable-option-id") || node.getAttribute("data-option-id"))) || ""',
    '    if (!optionId && node) {',
    ...
    '    if (!selectedNode) return',
    '    if (!isDeletableSelection(selectedNode, selectedKind)) return',
    // Snapshot every deletable target (primary + multi-selection extras)
    // BEFORE mutating — clearing the selection wipes extraSelectedNodes.
    '    var targets = [{ node: selectedNode, kind: selectedKind, label: (selectionLabelEl && selectionLabelEl.textContent) || "" }]',
    '    for (var xi = 0; xi < extraSelectedNodes.length; xi++) {',
    '      var xEntry = extraSelectedNodes[xi]',
    '      if (!xEntry.node || xEntry.node === selectedNode) continue',
    '      var xKind = xEntry.classification ? xEntry.classification.kind : "element"',
    '      if (!isDeletableSelection(xEntry.node, xKind)) continue',
    '      targets.push({ node: xEntry.node, kind: xKind, label: xEntry.classification ? (xEntry.classification.label || "") : "" })',
    '    }',
    '    var deletedAny = false',
    '    for (var ti = 0; ti < targets.length; ti++) {',
    '      var t = targets[ti]',
    '      var stableId = computeStableTextId(t.node)',
    ...
    '      if (textEditNode === t.node) { try { exitTextEditMode() } catch (e) {} }',
    '      var meta = buildHiddenOverrideMeta(t.node, t.kind, t.label)',
    '      applyHiddenToNode(t.node, stableId, { hidden: true, role: meta.role, optionId: meta.optionId, label: meta.label, cssLabel: meta.cssLabel, anchor: meta.anchor })',
    '      postCommittedHidden(stableId, t.node, t.kind, t.label)',
    '      deletedAny = true',
    '    }',
    ...
    '      pointerId: event.pointerId',
    '    }',
    // Multi-selection: every extra rides along with the same delta. Each
    // entry captures its own start offset and transition/will-change
    // baselines (enterDragFastMode only handles the primary node).
    '    dragState.extras = []',
    '    for (var xi = 0; xi < extraSelectedNodes.length; xi++) {',
    '      var xEntry = extraSelectedNodes[xi]',
    '      if (!xEntry.node || xEntry.node === selectedNode) continue',
    '      var xId = computeStableTextId(xEntry.node)',
    '      if (!xId) continue',
    '      var xCur = readCurrentOffsetFor(xId)',
    '      dragState.extras.push({',
    '        node: xEntry.node,',
    '        stableId: xId,',
    '        startDx: xCur.dx,',
    '        startDy: xCur.dy,',
    '        kind: xEntry.classification ? xEntry.classification.kind : "element",',
    '        label: xEntry.classification ? (xEntry.classification.label || "") : "",',
    '        transitionBaseline: (xEntry.node.style && xEntry.node.style.transition) || "",',
    '        willChangeBaseline: (xEntry.node.style && xEntry.node.style.willChange) || ""',
    '      })',
    '      if (xEntry.node.style) {',
    '        xEntry.node.style.transition = "none"',
    '        xEntry.node.style.willChange = "transform"',
    '      }',
    '    }',
    ...
    // Apply the same (snapped) delta to every extra in the multi-selection.
    '    if (dragState.extras && dragState.extras.length) {',
    '      var sharedDx = dx - dragState.startDx',
    '      var sharedDy = dy - dragState.startDy',
    '      for (var xi = 0; xi < dragState.extras.length; xi++) {',
    '        var ex = dragState.extras[xi]',
    '        applyPositionToNode(ex.node, ex.stableId, { dx: ex.startDx + sharedDx, dy: ex.startDy + sharedDy })',
    '      }',
    '    }',
    ...
    '    var extras = dragState.extras || []',
    '    try { event.target.releasePointerCapture && event.target.releasePointerCapture(event.pointerId) } catch (e) {}',
    '    exitDragFastMode(node)',
    '    for (var ri = 0; ri < extras.length; ri++) {',
    '      var rx = extras[ri]',
    '      if (rx.node && rx.node.style) {',
    '        rx.node.style.transition = rx.transitionBaseline',
    '        rx.node.style.willChange = rx.willChangeBaseline',
    '      }',
    '    }',
    '    hideSnapReferenceMarker()',
    '    dragState = null',
    ...
    '    var groupDx = dx - priorDx',
    '    var groupDy = dy - priorDy',
    '    for (var ci = 0; ci < extras.length; ci++) {',
    '      var cx = extras[ci]',
    '      var exDx = cx.startDx + groupDx',
    '      var exDy = cx.startDy + groupDy',
    '      applyPositionToNode(cx.node, cx.stableId, { dx: exDx, dy: exDy })',
    '      lastKnownPositionOverrides[cx.stableId] = { dx: exDx, dy: exDy }',
    '      postCommittedPosition(cx.stableId, cx.node, exDx, exDy, cx.startDx, cx.startDy, cx.kind, cx.label)',
    '    }',
    '    refreshSelectionOverlay()',
    '  }',
