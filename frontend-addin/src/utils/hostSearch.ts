...
export type HostSearchEventHit =
  ...
  for (const session of sessions) {
    ...
    for (const poll of snap.polls) {
      if (poll.question.toLowerCase().includes(q)) {
        ...
        if (hits.length >= MAX_EVENT_HITS) return hits
      }
    }
