...
export interface DeleteSessionConfirmModalProps {
  open: boolean
  /** Display title of the session about to be deleted (already trimmed/defaulted). */
  sessionTitle: string
  onCancel: () => void
...
/**
 * Confirmation before permanently deleting a session and all of its
 * activities/results. Mirrors DeleteActivityConfirmModal: portals to
 * `document.body` so the fixed backdrop covers the full viewport.
 */
      ...
      >
        <div className="px-7 pt-7 pb-2">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h2 id="delete-session-title" className="text-lg font-bold text-slate-900 !m-0">
              Delete session?
            </h2>
          </div>
          <p className="text-sm text-muted mt-2 leading-relaxed !m-0">
            You are about to delete{' '}
            <span className="font-bold text-slate-900">{sessionTitle}</span>. All of its
            activities and results will be permanently removed. This action{' '}
